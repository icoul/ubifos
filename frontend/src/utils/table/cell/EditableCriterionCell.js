    ...
    setData((data) => {
      return data.map(data => {
        return {
          ...data,
          criterionValueList: 
            (data.criterionIdx === idx ? criterions : data.criterionValueList)
        }
      })
    });
  }
          ...
          if (data.standType === id) {
            return (
              <div key={data.criterionValueIdx}>
                ...
                <EditableCriterionCell type="text" 
                                   value={data.standVal} 
                                   onChange={({target: { value }}) => { onChange(data.criterionValueIdx, 'standVal', value) }} 
                                   onBlur={() => { onBlur(data.criterionIdx) }} />
                <EditableCriterionCellSelect value={data.standRange} 
                        onChange={({target: { value }}) => { onChange(data.criterionValueIdx, 'standRange', value) }} 
                        onBlur={() => { onBlur(data.criterionIdx) }}>
