...
export const columns = (criterion) => {
  return [
    ...
    },
    {
      id: "modelNm",
      Header: () => <div>장치명</div>,
      Cell: ({ row }) => <>{row.original.module.modelNm}</>,
    },
    {
      id: "status",
      Header: () => <div>상태</div>,
      Cell: ({ row }) =>
        checkGasStatus(row.original, criterion) === "danger" ? (
    ...
    {
      id: "battery",
      Header: () => <div>battery</div>,
      Cell: ({ row }) => <>{Number(row.original.battery)} %</>,
    },
    {
      id: "rgstDt",
      Header: () => <div style={{ width: "100px" }}>날짜</div>,
      Cell: ({ row }) => (
...
export const checkGasStatus = (checkTheLog, criterions) => {
  ...
  gasCriterionMap.forEach((x) => {
    criterions[x]
      .filter((y) => y.standType !== "criterion")
      .forEach((gasCriterion) => {
        const check = checker(
          checkTheLog[x],
          gasCriterion.standType,
          gasCriterion.standVal,
          gasCriterion.standRange
        );
