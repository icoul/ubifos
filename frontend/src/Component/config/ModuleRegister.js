...
const ModuleRegister = () => {
  const [ formBoxFlag, setFormBoxFlag ] = useState(true);
  ...
  return (
    <ButtonBarConatiner>
      <div className="buttonBar-container">
        <button className="search-condition csv-btn col-xl-4 col-sm-6" onClick={() => { handleClick(); }}>{(formBoxFlag ? "등록" : "등록취소")}</button>
      </div>
      <FormContainer>
        <div className="form-container" style={{display:(formBoxFlag ? 'none' : 'block')}}>
